...
public class IppServer : BackgroundService
{
    ...
    /// <summary>
    /// Get-Printer-Attributes — самый важный запрос.
    /// iPhone задаёт его ещё до попытки печати, чтобы понять что умеет принтер.
    /// Если ответ некорректный или не содержит нужных атрибутов — принтер будет недоступен.
    /// </summary>
    /*private byte[] BuildGetPrinterAttributesResponse(int requestId)
    {
        ...
        // Эта группа обязательна в каждом ответе, содержит базовые мета-атрибуты
        ...
        var printerUri = $"ipp://localhost:{_config.IppPort}/{_config.ResourcePath}";
        ...
        // ipp-versions-supported — говорим что умеем 1.1 и 2.0
        w.WriteAttribute(ValueTagKeyword, "ipp-versions-supported", "1.1");
        w.WriteAttributeAdditional(ValueTagKeyword, "2.0");

        // Список операций которые мы поддерживаем (в виде enum-значений из RFC 8011)
        ...
        w.WriteIntAttribute("multiple-document-jobs-supported", ValueTagBoolean, 0);
        ...
        // document-format-default и supported — что умеем принимать
        ...
        w.WriteAttributeAdditional(ValueTagMimeType, "image/pwg-raster");

        w.WriteIntAttribute("printer-is-accepting-jobs", ValueTagBoolean, 1);
        w.WriteIntAttribute("queued-job-count", ValueTagInteger, 0);

        // pdl-override-supported — можем ли переопределять формат
        w.WriteAttribute(ValueTagKeyword, "pdl-override-supported", "not-attempted");
    ...
    private byte[] BuildGetPrinterAttributesResponse(int requestId)
    {
        ...
        var printerUri = $"ipp://{GetLocalIp()}:{_config.IppPort}/{_config.ResourcePath}";

        // CRITICAL ATTRIBUTES для AirPrint:
        w.WriteAttribute(ValueTagUri, "printer-uri-supported", printerUri);
        w.WriteAttribute(ValueTagKeyword, "uri-security-supported", "none");
        w.WriteAttribute(ValueTagKeyword, "uri-authentication-supported", "none");
        w.WriteAttribute(ValueTagNameNoLang, "printer-name", _config.DisplayName);
        w.WriteAttribute(ValueTagTextNoLang, "printer-info", "Windows Shared Printer");
        w.WriteAttribute(ValueTagTextNoLang, "printer-location", "");
        ...
        // НОВЫЕ КРИТИЧЕСКИЕ АТРИБУТЫ:

        // Цвет
        w.WriteIntAttribute("color-supported", ValueTagBoolean, 1);

        // Copies
        w.WriteIntAttribute("copies-supported", ValueTagRangeOfInt, 1);
        w.WriteIntAttributeAdditional(ValueTagRangeOfInt, 99);
        w.WriteIntAttribute("copies-default", ValueTagInteger, 1);

        // Ориентация
        w.WriteIntAttribute("orientation-requested-supported", ValueTagEnum, 3); // portrait
        w.WriteIntAttributeAdditional(ValueTagEnum, 4); // landscape
        w.WriteIntAttribute("orientation-requested-default", ValueTagEnum, 3);

        // Качество
        w.WriteIntAttribute("print-quality-supported", ValueTagEnum, 3); // draft
        w.WriteIntAttributeAdditional(ValueTagEnum, 4); // normal
        w.WriteIntAttributeAdditional(ValueTagEnum, 5); // high
        w.WriteIntAttribute("print-quality-default", ValueTagEnum, 4);

        // Разрешение
        WriteResolutionAttribute(w, "printer-resolution-supported", 600, 600, 3); // 600dpi, dots per inch
        WriteResolutionAttribute(w, "printer-resolution-default", 600, 600, 3);

        // Размеры бумаги (media)
        ...
        w.WriteAttributeAdditional(ValueTagKeyword, "na_legal_8.5x14in");
        w.WriteAttribute(ValueTagKeyword, "media-default", "iso_a4_210x297mm");

        // Поля страницы
        w.WriteIntAttribute("media-left-margin-supported", ValueTagInteger, 0);
        w.WriteIntAttribute("media-right-margin-supported", ValueTagInteger, 0);
        w.WriteIntAttribute("media-top-margin-supported", ValueTagInteger, 0);
        w.WriteIntAttribute("media-bottom-margin-supported", ValueTagInteger, 0);

        // Источник бумаги
        w.WriteAttribute(ValueTagKeyword, "media-source-supported", "auto");
        w.WriteAttribute(ValueTagKeyword, "media-source-default", "auto");

        // Тип бумаги
        w.WriteAttribute(ValueTagKeyword, "media-type-supported", "stationery");
        w.WriteAttributeAdditional(ValueTagKeyword, "photographic");
        w.WriteAttribute(ValueTagKeyword, "media-type-default", "stationery");
        ...
        w.WriteIntAttribute("job-priority-supported", ValueTagInteger, 100);
        w.WriteIntAttribute("job-priority-default", ValueTagInteger, 50);
        w.WriteIntAttribute("job-hold-until-supported", ValueTagKeyword, 0);
        w.WriteAttribute(ValueTagKeyword, "job-hold-until-default", "no-hold");
        w.WriteIntAttribute("job-sheets-supported", ValueTagKeyword, 0);
        w.WriteAttribute(ValueTagKeyword, "job-sheets-default", "none");

        // Multiple document handling
        w.WriteIntAttribute("multiple-document-handling-supported", ValueTagKeyword, 0);
        w.WriteAttribute(ValueTagKeyword, "multiple-document-handling-default", "separate-documents-uncollated-copies");
